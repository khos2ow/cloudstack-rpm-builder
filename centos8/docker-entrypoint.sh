...
if [ ! -f "${workspace_path}/packaging/package.sh" ]; then
    echo "Could not find '${workspace_path}/packaging/package.sh'"
    exit 1
fi
...
# Show help, both from docker-entrypoint.sh and ${workspace_path}/packaging/package.sh
if [ $show_help = true ]; then
...

{
    ...
    bash -x ./package.sh $@ && {
        # create RPMs
        createrepo ${workspace_path}/dist/rpmbuild/RPMS
        adjust_owner
    }
} || {
    status=$?
