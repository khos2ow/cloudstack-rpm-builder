...
# Make sure package.sh script exists before going any further
if [ ! -f "/mnt/build/cloudstack/packaging/package.sh" ]; then
    echo "Could not find '/mnt/build/cloudstack/packaging/package.sh'"
    exit 1
fi
...
if [ $major_version -lt 4 ] || [ $major_version -eq 4 -a $minor_version -lt 12 ]; then
    ...
    for arg in "$@"; do
        if [ -n "$arg" ]; then
            case "$arg" in
                --distribution)  PKG_ARGS="$PKG_ARGS -d" ;;
                --pack)          PKG_ARGS="$PKG_ARGS -p" ;;
                --release)       PKG_ARGS="$PKG_ARGS -r" ;;
                --simulator)     PKG_ARGS="$PKG_ARGS -s" ;;
                -* | *)          PKG_ARGS="$PKG_ARGS $arg" ;;
            esac
        fi
    done
...
# Show help, both from docker-entrypoint.sh and /mnt/build/cloudstack/packaging/package.sh
if [ $show_help = true ]; then
    help=$(cd /mnt/build/cloudstack/packaging; bash -x ./package.sh $HELP_ARG)
    cat << USAGE
